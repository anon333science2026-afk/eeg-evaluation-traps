#!/usr/bin/env python3
"""
figA.1.7.1 — mismatch variants (no suptitle).
2×2 layout: two cases showing mismatch runs both ways.
  Row 1: MLP/T-test — epoch overestimates subject (Δ = −24.5 pp)
  Row 2: KNN k=7/T-test — subject overestimates epoch (Δ ≈ +32.8 pp)
Symbols fixed: $\checkmark$ / $\\times$ (mathtext, no missing-glyph boxes).
"""
import pandas as pd
import numpy as np
import matplotlib
...
# Case 1: MLP/T-test, epoch > subject (largest positive mismatch)
case1 = m6[m6["pipeline"] == "FTest"].nlargest(1, "mismatch").iloc[0]

# Case 2: KNN k=7/T-test, subject > epoch (largest negative mismatch with k=7)
case2 = (m6[(m6["model"] == "KNN") &
            (m6["hyperparams"].str.contains('"n_neighbors": 7'))]
         .copy())
case2 = case2[case2["mismatch"] < 0].nsmallest(1, "mismatch").iloc[0]
...
def prep_fold(fp):
    ...
    # Sort: AD (class 1) first, then Control (class 0); within each group by n_epochs desc
...
def draw_bars(ax, subjects, true_labels, correct, n_epochs,
              correct_votes, other_votes, model_label, pipeline_label):
...
# Row 1 — MLP / T-test (epoch overestimates)
...
draw_bars(axes[0, 0], s, tl, c, ne, cv, ov, "MLP", "T-test")
draw_accuracy(axes[0, 1], epoch1, subject1, delta1)

# Row 2 — KNN k=7 / T-test (subject overestimates)
...
draw_bars(axes[1, 0], s2, tl2, c2, ne2, cv2, ov2, "KNN (k=7)", "T-test")
draw_accuracy(axes[1, 1], epoch2, subject2, -(delta2))   # negative so fill logic works

plt.tight_layout()
plt.subplots_adjust(top=0.93)

# Horizontal row titles placed above each row in figure coordinates
...
plt.close(fig)
print(f"Saved -> {out}")
print(f"Case 1 (MLP, epoch>subj): epoch={epoch1:.1f}%  subj={subject1:.1f}%  Δ={delta1:+.1f} pp")
