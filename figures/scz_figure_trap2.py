...
LAYOUT = [
    ("PCA_L_6_SCZ",   "PCA - P=6"),
    ("PCA_L_2_SCZ",   "PCA - P=2"),
    ("ANOVA_L_6_SCZ", "T-test - P=6"),
    ("ANOVA_L_2_SCZ", "T-test - P=2"),
]
...
def main():
    ...
    # 2×2: top row = PCA (P=6, P=2), bottom row = T-test (P=6, P=2)
    panel_map = [
        (0, 0, "PCA_L_6_SCZ",   "PCA — P=6"),
        (0, 1, "PCA_L_2_SCZ",   "PCA — P=2"),
        (1, 0, "ANOVA_L_6_SCZ", "T-test — P=6"),
        (1, 1, "ANOVA_L_2_SCZ", "T-test — P=2"),
    ]

    for row, col, exp_name, title in panel_map:
        data = load_best_hp(df, exp_name)
        draw_panel(axes[row, col], data, CHANCE_EPOCH)
        axes[row, col].set_title(title, fontsize=14, fontweight="bold", pad=6)
        if col == 0:
            axes[row, col].set_ylabel("Epoch accuracy", fontsize=13)
    ...
    outfile = OUTDIR / "scz_trap2_CCN.png"
