#!/usr/bin/env python3
"""
FTD vs C — Trap 1 CCN figure (both PCA and ANOVA, KNN=7).
...
Dataset: FTD vs Control
  ...
  Subject-level majority-class chance: 29/52 = 0.558 (control)
  Epoch-level chance: 0.5422 (control majority at epoch level)
...
BASE_FTD  = Path(f"{REPO}/processed_data/results/ftd")
INTRA_DIR = BASE_FTD / "intra-subject_seed_runs"
DATA_CSV  = BASE_FTD / "ftd_all_experiments_combined.csv"
...
def load_wc_direct(feature: str) -> dict:
    ...
    for seed in SEEDS:
        dname = f"{feature}_W_C_ftd_vs_cntrl_seed{seed}"
        ml_dir = INTRA_DIR / dname / "ml_results_grid_search"
...
def draw_panel(ax, left_vals, right_vals, col_L, col_R,
    ...
    for mi, model in enumerate(MODEL_ORDER):
        ...
        # overlap on LEFT, disjoint on RIGHT — funnel jitter (tight at extremes)
...
outfile = OUTDIR / "ftd_trap1_CCN.png"
